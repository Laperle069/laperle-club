...
export async function walletService(req:Request,deps:Dependencies):Promise<Response|null> {
 ...
 if(path==='/check') {
  ...
  const readiness=await deps.check?.();
  if(readiness?.signingReady===false) return response(readiness,503);
  ...
  return response({...readiness,apnsReady:result.status===400&&result.reason==='BadDeviceToken',status:result.status,reason:result.reason});
 }
 if(path==='/sync') {
  if(req.method!=='POST') return empty(405);
  if(!await deps.authorized(req.headers.get('x-sync-geheimnis')??'')) return empty(401);
