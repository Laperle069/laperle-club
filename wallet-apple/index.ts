import { refinedAssets } from "./refined-artwork.ts";
...
async function ladeBilder(rang:string|null):Promise<Record<string,string>> {
  ...
  const entries=await Promise.all(assetNamen(rang).map(async name=>{
    if (refinedAssets[name]) return [name,refinedAssets[name]];
...
async function check() {
 if(!signing().key) return {signingReady:false,reason:"APPLE_SIGNER_KEY fehlt"};
