...
(() => {
 ...
 function renderRewards(d){
  ...
  for(const p of all){const b=node('button',null,'item'+(p.erreichbar?' frei':''));b.type='button';b.id='praemie-'+p.id;b.disabled=!p.erreichbar;b.append(node('span',number(p.punkte)+' Perlen','pts'),node('strong',p.bezeichnung));
 ...
 function renderRank(d){const l=d.level;rankCard.hidden=!l||d.funktionen?.level===false;if(rankCard.hidden)return;
  ...
  for(const r of l.alle||[]){const item=node('li',null);if(r.stufe===l.stufe)item.setAttribute('aria-current','true');item.append(node('strong',r.name+(r.stufe===l.stufe?' · dein Rang':'')),node('span','Ab '+number(r.ab_punkten)+' gesammelten Rangperlen'),node('small',r.vorteil||'Deine Clubvorteile'));list.append(item);}
 ...
 const wordmark=node('figcaption',null,'lp-pearl-wordmark');wordmark.append(node('span','La Perlé'),node('small','Beauty Boutique'));club.querySelector('.lp-pearl-object')?.append(wordmark);
 ...
 function updateMission(){
  ...
  if(!current||current.abgeschlossen){
   ...
   if(options.length){const form=node('form',null),fieldset=node('fieldset',null);fieldset.append(node('legend','Wähle dein Ziel'));
 ...
 function render(d){renderRewards(d);renderRank(d);renderAdvent(d);demoEditor();loadMissions();}
