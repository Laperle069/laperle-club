...
(() => {
 ...
 function apply(value,persist=false){theme=value;root.dataset.lpAppearance=value;document.querySelector('meta[name="theme-color"]')?.setAttribute('content',value==='dark'?'#21191A':'#FFFDF9');document.querySelectorAll('.lp-palette button').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.theme===value)));if(persist)try{localStorage.setItem(key,value);saved=value;}catch{}document.dispatchEvent(new Event('lp:themechange'));}
 apply(theme);
