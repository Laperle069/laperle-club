...
(async()=>{
 ...
 const layout=JSON.parse(r.calls.find(x=>x.url.includes('/loyaltyClass/')&&x.init.method==='PATCH').init.body).classTemplateInfo.cardTemplateOverride;
 assert.equal(layout.cardRowTemplateInfos.length,3);assert.equal(layout.cardRowTemplateInfos[0].oneItem.item.firstValue.fields[0].fieldPath,'object.accountName');assert.equal(layout.cardRowTemplateInfos[1].oneItem.item.firstValue.fields[0].fieldPath,"object.textModulesData['mitglied_seit']");
 console.log('PASS Google: eigener Namensbereich, Mitglied seit darunter, Punkte und Rangziel in dritter Zeile');
 const obj=JSON.parse(r.calls.find(x=>x.url.endsWith('/loyaltyObject')).init.body);
 assert.equal(obj.barcode.value,card.kundennummer);assert.ok(!('hexBackgroundColor' in obj));assert.equal(obj.secondaryLoyaltyPoints.balance.string,'Höchster Rang erreicht');console.log('PASS Echte Kunden-, Punkte- und Rangdaten im Google-Objekt');
